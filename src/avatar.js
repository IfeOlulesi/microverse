...
export class AvatarPawn extends mix(CardPawn).with(PM_Player, PM_SmoothedDriver, PM_ThreeVisible, PM_ThreeCamera, PM_Pointer) {
        ...
        this.shellListener = (command, { frameType, spec, cameraMatrix, dx, dy, acknowledgeReceipt }) => {
            switch (command) {
                case "frame-type":
                    ...
                    // a frame generated by addFrame supplies no spec, and doesn't need to
                    // await unfreezing of rendering.
                    if (spec) this.frozenForFrameSwitch = true;
                ...
                case "release-freeze":
                    ...
                    this.updatePortalRender(true); // true => force release of freeze
                    break;
                case "sync-render-now":
                    if (this.frozenForFrameSwitch) {
                ...
                case "portal-camera-update":
                    ...
                    this.frozenForFrameSwitch = false;
    ...
    specForPortal(portal, jumpVector) {
    ...
    // this is sent on every update().  if this is the primary frame and it has portals,
    ...
    updatePortalRender(releaseFreeze = false) {
        if (this.frozenForFrameSwitch && !releaseFreeze) return; // all rendering is suppressed
        ...
        if (this.isPrimary) {
            ...
            // if we were frozen for a world switch, and it turns out that there
            // are no portals whose rendering the shell will await, render immediately
            // and send "primary-rendered" to the shell so it will reorder any frames.
            if (this.frozenForFrameSwitch && !secondaryRenders) {
    ...
    portalCameraUpdate(cameraMatrix) {
        ...
        if (cameraMatrix) {
            ...
            this.endMovementTimeout = setTimeout(() => {
                const renderNow = !!this.lastCameraMatrix && !this.frozenForFrameSwitch;
            ...
            if (!this.isPrimary && !this.frozenForFrameSwitch) {
                this.refreshCameraTransform();
                renderMgr.composer.render();
    ...
    collidePortal(vq) {
        if (this.frozenForFrameSwitch) return false;
        ...
        if (firstIntersection) {
            ...
            this.frozenForFrameSwitch = true;
            ...
            const transferData = this.specForPortal(portal, jumpVector);
            if (crossingBackwards) transferData.crossingBackwards = true;
